...
export function obsidianCalloutsPlugin(md: MarkdownIt) {
  ...
  md.core.ruler.push('obsidian_callout', (state) => {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (!firstContentToken || firstContentToken.type !== 'paragraph_open') continue
      ...
      if (inlineToken && inlineToken.type === 'inline' && inlineToken.content) {
        ...
        if (match && match[1] && match[3] !== undefined) {
          ...
          if (closeToken) {
            closeToken.type = 'callout_details_close'
            closeToken.tag = 'details'
          }
          ...
          if (inlineToken.children) {
            ...
            if (kids.length === 0) {
              ...
              const tok1 = tokens[firstContentIndex]
              const tok2 = tokens[firstContentIndex + 1]
              const tok3 = tokens[firstContentIndex + 2]
              if (tok1) {
                tok1.type = 'hidden'
                tok1.hidden = true
              }
...
function findMatchingCloseToken(tokens: any[], startIndex: number): number {
  ...
  for (let i = startIndex; i < tokens.length; i++) {
    ...
    if (token && token.type === 'blockquote_open') {
      level++
    } else if (token && token.type === 'blockquote_close') {
      level--
      if (level === 0) {
        return i
      }
